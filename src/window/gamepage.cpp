...
void GamePage::init() {
  ...
  redTankItem->setTransformOriginPoint(redTankItem->pixmap().width() / 2,
                                       redTankItem->pixmap().height() / 2);
  redTankItem->setPos(RED_TANK_INIT_X, RED_TANK_INIT_Y);
  scene->addItem(redTankItem);

  greenTankItem =
      new QGraphicsPixmapItem(QPixmap(":/pic/pictures/Tank_Green.png"));
  greenTankItem->setTransformOriginPoint(greenTankItem->pixmap().width() / 2,
                                         greenTankItem->pixmap().height() / 2);
...
void GamePage::attach_redTank(TPoint *tank) { this->redTank = tank; }

void GamePage::attach_greenTank(TPoint *tank) { this->greenTank = tank; }
...
void GamePage::attach_walls(QVector<Wall *> *walls) {
  this->walls = walls;
  for (auto wall : (*walls)) {
    QGraphicsRectItem *wallItem = new QGraphicsRectItem(
...
void GamePage::get_Notification(Item category, Notification nId) {
  if (nId == TANK_MOVE_FORWARD || nId == TANK_MOVE_BACKWARD) {
    if (category == RED_TANK)
      ...
      this->greenTankItem->setPos(this->greenTank->getX(),
                                  this->greenTank->getY());
    ...
    for (int i = 0; i < bullets->size(); i++) {
      ...
      if ((*bullets).at(i)->isAvailable())
        bulletItems[i]->setVisible(false);
      else
        bulletItems[i]->setVisible(true);
    }
  }
  if (nId == SCORE_CHANGE) {
    // qDebug() << " nId " << nId;
...
void GamePage::keyPressEvent(QKeyEvent *event) {
  ...
  if (event->key() == Qt::Key_W || event->key() == Qt::Key_S ||
      ...
      event->key() == Qt::Key_Right || event->key() == Qt::Key_M ||
      event->key() == Qt::Key_R) {
